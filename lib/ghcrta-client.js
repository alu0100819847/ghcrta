  ...
  renderLogIn(container_, err){
    ...
    ReactDom.render(
      <div style={{position: 'relative', top: 20+'%'}}>
        <h1 style={{ textAlign: 'center'}}> Github Login </h1>
        <div key="login" style={{border: 1+'px solid', margin: 3+'px', padding: 5+'px'}}>
          <div style={{ textAlign: 'center'}}><label> <div style={{display: 'inline-block',marginRight: 2.85+'em'}}>Name: </div><input id="name" type="text"></input></label></div>
          <div style={{ textAlign: 'center'}}><label> <div style={{display: 'inline-block',marginRight: 1+'em'}}>Password: </div><input id="password" type="password"></input></label></div>
          <div style={{ width: 100+'%'}}> <button style={{position: 'relative', left: 45+'%'}} onClick={() => {this.getLogin(container_)}}>Login</button> </div>
        </div>
    ...
    logedIn(container_){
      if(this.client){
        ...
        orgs_promise.then((response) => {
          let style = {'buttons': {position: 'absolute', top: 95+'%',padding: 3+'px', maxHeight: 5+'%', width: 100+'%', border: '1px solid', backgroundColor: '#d8d8d8', color: 'black', textAlign: 'center'}, 'search':{borderBottom: 1+'px solid' ,padding: 3+'px', color: 'black', backgroundColor: '#d8d8d8'}}
          ReactDom.render(
            <div>
              <div style={style.search} id='search'></div>
...
// *************
  paginatedOrgs(pag){
    ...
    let style = {'orgs': {overflow: 'scroll', height: atom.getSize().height*0.7 + 'px', marginTop: 1.5+ 'em' }, 'list':{backgroundColor: '#d8d8d814', margin: 3+'px', border: 1+'px', borderRadius: 20+'px', padding: 5+'px'}}
    var list = []
    if(this.actualOrgs[pag]){
      for(var i = 0; i < this.actualOrgs[pag].length; i++){
        list.push(<li style={style.list} key={'org-'+i} id={'org-'+i}></li>)
      }
      while(list.length < this.paginating){
        list.push(<li key={'org-'+list.length} id={'org-'+list.length} hidden></li>)
      }
      let prev = <a key='prev' style={{color: 'black'}} onClick={this.getOrgs(pag-1)}>&#8678;</a>
      let next = <a key='next' style={{color: 'black'}} onClick={this.getOrgs(pag+1)}>&#8680;</a>
  ...
  getOrgRepos(org, pag, container_){
    ...
    return () =>{
      ...
      let root = document.getElementById('container');
      let style = {'back' : {padding: 1 + 'em', borderBottom: 1+'px solid'}, 'repos': {overflow: 'scroll', height: atom.getSize().height*0.65 + 'px' }}
      ReactDom.render(
        <div>
