      ...
      if(response.data){
        ...
        ReactDom.render(
            <div style={{margin: 5 + 'px'}}>
              ...
              <a id='dirBut' style={{margin: 5+'px', border: 1+'px solid', backgroundColor: 'green'}} onClick={() => {atom.commands.dispatch(container, 'ghcrta:dir')}}>DIR</a>
              <div style={{display: 'inline', color: 'green'}} id='countErr'>{GhcrtaDir.error.length}</div>
            </div>
          ,
          searcher
        )
  ...
  paginatedRepos(pag){
    ...
    if(this.actualRepos[pag]){
      ...
      for(var i = 0; i < this.actualRepos[pag].length; i++){
        list.push(<li style={{backgroundColor: '#d8d8d814', margin: 3+'px', border: 1+'px', borderRadius: 20+'px', padding: 5+'px'}} key={'rep-'+i} id={'rep-'+i}></li>)
      }
